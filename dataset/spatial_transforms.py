...
class Salt(object):
    ...
    def __call__(self, imgs: Union[Tensor, Image.Image]) -> Union[Tensor, Image.Image]:
        ...
        if isinstance(imgs, Image.Image):
            imgs = TF.to_tensor(imgs) #Converts an image to tensor if needed.

        orig_shape = imgs.shape
        imgs = torch.reshape(imgs, shape=[-1]) #Reshapes the tensor to a 1D array.


        if int(self.saltness) != self.saltness:
            num_salts = int(self.saltness * len(imgs)) #Computes the number of pixels to salt based on `saltness`.
